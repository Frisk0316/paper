...
provider.
The base file, \texttt{crypto\_panel\_features.npz}, is a processed weekly
...
The augmented file, \texttt{crypto\_panel\_features\_with\_etf.npz}, appends
...
\scriptsize
\begin{tabularx}{\linewidth}{p{0.21\linewidth}X p{0.16\linewidth}X p{0.12\linewidth}}
\toprule
\thead{File} & \thead{Variables / role} & \thead{Period} &
\thead{Construction step} & \thead{Raw source observable?} \\
\midrule
\texttt{crypto\_panel\_features.npz}
  ...
  & No \\
\texttt{crypto\_panel\_features\_with\_etf.npz}
