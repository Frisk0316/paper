...
with a minimum of $-0.78$ and maximum of $+2.51$.
The 32-seed ensemble Sharpe ratio is $+2.46$—3.5$\times$ the per-seed
...
is only $+0.26$, yet the ensemble achieves $+2.13$ (8.2$\times$
amplification).
...
Hyperparameters $\alpha \in \{0.001, 0.01, 0.1, 1.0\}$ and
$\ell_1\text{-ratio} \in \{0.1, 0.5, 0.9, 1.0\}$ are selected by
...
Five independent forests (different \texttt{random\_state}) are trained
...
As with RF, predictions are averaged over 5 seeds.
...
The asymmetry between DL (32 seeds), tree models (5 seeds), and linear
