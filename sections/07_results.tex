...
\input{tables/table_cross_model}
...
The corrected full-scale VSN—with $\sim$27K parameters versus the
lightweight version's $\sim$5K—contributes to this recovery by providing
richer per-feature gating, consistent with the original
\citet{lim2021temporal} specification.
...
ElasticNet produces a stable but narrow band of Sharpe ratios
($\approx$1.0--1.3) across configurations and weighting schemes,
consistent with the hypothesis that strong L1 regularization concentrates
the model on a handful of momentum and on-chain signals and effectively
ignores the remaining features; the repeated values marked $^{*}$ in
\Cref{tab:cross_model} likely reflect a highly concentrated coefficient
structure rather than materially different fitted portfolios.
OLS tracks ElasticNet closely, confirming that linear models without
sparsity penalties reach similar performance ceilings.
...
\begin{figure}[t]
  ...
  \caption{Cumulative long-short returns for the best configuration of
           ...
           TFT \texttt{+Trump+ETF}, and RF \texttt{All}).}
...
CS-Gated and LSTM deliver the strongest final outcomes, whereas RF remains
competitive but appears more episodic, which is consistent with its
configuration-specific dominance in \Cref{tab:cross_model} rather than
uniform superiority across rows.
...
\begin{figure}[t]
  ...
  \caption{Sharpe ratio of each predicted-rank decile portfolio for
           CS-Gated (\texttt{+Onchain}, EW). Decile 1 corresponds to the
           short leg (lowest predicted rank); decile 10 corresponds to the
           long leg (highest predicted rank).}
